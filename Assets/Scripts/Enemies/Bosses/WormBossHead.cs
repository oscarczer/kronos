...
public class WormBossHead : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (rigidBody.linearVelocity.magnitude < 1 || transform.position.x > 35 || transform.position.y > 6 || transform.position.x < -35 || transform.position.y < -30) {
        ...
        if(health <= 0 & !isDead) {
            ...
            first.GetComponent<WormBossHead>().health = first.GetComponent<WormBossBody>().health < 15 ? 2 * first.GetComponent<WormBossBody>().health : 30;
